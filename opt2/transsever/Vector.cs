...
namespace Opt
{
	class Vector
	{
		...
		public static Vector operator *(Matrix matrix, Vector v)
		{
			int col = matrix.Cols;
			int rows = v.size;
			Vector rez = new Vector(v.size);
		...
		{
			Vector rez = new Vector(v.size);
			for (int i = 0; i < v.size; i++) rez.data[i] = c/ v.data[i] ;
			return rez;
		...
		public static Vector operator +(Vector u, Vector v)
		{
			...
			Vector rez = new Vector(v.size);
			for (int i = 0; i < v.size; i++) rez.data[i] = v.data[i] + u.data[i];
			return rez;

		}
		public static Vector operator +(double u, Vector v)
		{
			
			Vector rez = new Vector(v.size);
			for (int i = 0; i < v.size; i++) rez.data[i] = v.data[i] + u;
			return rez;
